...
class InstagramAPIError(Exception):
    ...
    def __init__(self, message, status_code=None, fb_error_code=None, error_subcode=None):
...
class InstagramClient:
    ...
    def verify_account(cls, access_token: str) -> str:
        ...
        In Instagram Graph API, a user token provides access to FB Pages, and each 
        FB Page is linked to an Instagram Business Account.
        ...
        if cls.is_mock_token(access_token):
            ...
            return "17841401234567890"  # Dummy Instagram Business Account ID

        try:
            # Step 1: Get Facebook Pages managed by this user token
            pages_url = f"{cls.BASE_URL}/me/accounts"
            response = requests.get(pages_url, params={"access_token": access_token}, timeout=15)
            ...
            for page in pages_data:
                page_id = page.get("id")
                page_token = page.get("access_token")  # Use page access token if needed, or user token
    ...
    def create_media_container(
        ...
        For videos, is_video=True will include required video parameters.
        
        Returns:
            str: The media container creation ID.
        """
        if cls.is_mock_token(access_token):
    ...
    @classmethod
    def check_video_status(cls, container_id: str, access_token: str) -> dict:
        """
        Check the status of a video container. Video processing takes time.
        """
        if cls.is_mock_token(access_token):
        ...
        try:
            ...
            if response.status_code != 200:
                error_data = response.json().get("error", {})
                raise InstagramAPIError(f"Failed to check video status: {error_data.get('message')}")
            return response.json()
        except requests.exceptions.RequestException as e:
            raise InstagramAPIError(f"Network error checking video status: {str(e)}")

    @classmethod
    def wait_for_video_processing(cls, container_id: str, access_token: str, timeout: int = 180, interval: int = 10):
        """
        Poll the video container status until processing completes or fails.
        """
        if cls.is_mock_token(access_token):
            time.sleep(1)
            return
        ...
        while elapsed < timeout:
            status_info = cls.check_video_status(container_id, access_token)
            status_code = status_info.get("status_code")
            logger.info(f"Video container {container_id} status: {status_code}")

            if status_code == "FINISHED":
                return
            elif status_code in ["ERROR", "EXPIRED"]:
                error_msg = status_info.get("error_message", "Unknown video processing error.")
                raise InstagramAPIError(f"Video container processing failed: {error_msg}")

            time.sleep(interval)
            elapsed += interval

        raise InstagramAPIError("Video processing timed out on Instagram servers.")
    ...
    def publish_media_container(cls, instagram_business_id: str, creation_id: str, access_token: str) -> str:
        ...
        Returns:
            str: The published Instagram media object ID.
        """
        if cls.is_mock_token(access_token):
